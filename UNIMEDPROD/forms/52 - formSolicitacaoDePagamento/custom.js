...
$(document).ready(function(){
	...
	$("#cpfPessoaFisica").on('blur',function(){
		...
		if (this.value == null || this.value == '' || this.value == undefined) {
			MensagemAlerta('Solicitação de Pagamento', 'Favor informar o CPNJ/CPF do Fornecedor');
		} else 
			if ($('#zoomUnidadePesq').val() == '' || $('#zoomUnidadePesq').val() == null || $('#zoomUnidadePesq').val() == undefined) {
				MensagemAlerta('Solicitação de Pagamento', 'Favor informar Unidade para pesquisa de Fornecedor');
			} else {
	...
	$("#f_divDtVenc").on('blur change',function(){
		$('.has-error').removeClass('has-error');
	...
	$("#rateio").on('click change',function(){
...
function consultaFornecedor(valor) {
	...
	if (fornecedor.values.length > 0) {
		...
		$('#btnPesquisar').addClass('disabled');
		MensagemAlerta('Solicitação de Pagamento', 'Fornecedor não existe para essa Filial. Caso deseja adicioná-lo, favor clicar no botão "Adicionar".');
		return 3;
	}
}
